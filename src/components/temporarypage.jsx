...
const Page = () => {
  ...
  const scrambleEffect = useCallback((onComplete) => {
    ...
    const totalDuration = 1200; // Total duration of the scramble effect in ms
    const intervalDuration = 100; // Duration between each update
    ...
    <>
      ...
      <div className="bg-black h-screen flex flex-col justify-center items-center relative">
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center">
          ...
          <div
            className={`bg-White-gradient bg-clip-text text-transparent text-[14rem] font-extrabold font-akira leading-none tracking-[-0.07em] px-4 transition-opacity duration-500 ${
              showXXV ? 'opacity-100' : 'opacity-0'
            }`}
          >
            XXV
          </div>
          <div
            className={`text-[5rem] typewriter transition-opacity duration-500 ${
...
//this page is of no use just a temporary page for checks, please ignore it it'll be removed by the time site is in production.
// page.css is in component folder for the time being

//TODO:
//page.css will be moved to asset folder 